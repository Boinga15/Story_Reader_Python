...
class FifteenAPI:
    ...
    def get_tts_raw(self, character, text):
        ...
        if text_len > self.max_text_len:
            self.logger.warning('Text too long ({text_len} > {self.max_text_len}), trimming to {self.max_text_len} symbols')
        ...
        self.logger.info('Target text: [{text}]')
        self.logger.info('Character: [{character}]')
        ...
        try:
            ...
            resp["status"] = "ConnectionError ({e})"
            self.logger.error("ConnectionError ({e})")
            return resp
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                self.logger.info("15.ai API response success")
                return resp
            except requests.exceptions.ConnectionError as e:
                resp["status"] = "ConnectionError ({e})"
                self.logger.error("ConnectionError ({e})")
            ...
            self.logger.error('15.ai API request error, Status code: {response.status_code}')
            resp["status"] = '15.ai API request error, Status code: {response.status_code}'
        return resp
    ...
    def save_to_file(self, character, text, filename=None):
        ...
        if tts["status"] == "OK" and tts["data"] is not None:
            if filename is None:
                ...
                filename = "15ai-{char_filename_part}-{text_filename_part}-{round(time.time())}.wav"
